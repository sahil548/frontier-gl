...
import { getConsolidatedIncomeStatement } from "@/lib/queries/consolidated-report-queries";
...
export async function GET(request: Request) {
  ...
  try {
    const data = await getConsolidatedIncomeStatement(
      entityIds,
      new Date(startDate),
      new Date(endDate),
    ...
    console.error("Consolidated income statement error:", error);
